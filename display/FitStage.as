package com.kaka.display
{
        ...
        public class FitStage extends EventDispatcher
        {
                ...
                public function setMinSizeLimit(_w:Number,_h:Number)
                {
                        minw = _w;
                        minh = _h;
                }
                public function setMaxSizeLimit(_w:Number,_h:Number)
                {
                        maxw = _w;
                        maxh = _h;
                }
                ...
                private function onStageResize(e:Event)
                {
                        for (var i:Number = 0; i <FitObjectArray.length; i++)
                        {
                                ...
                                if (theStage.stageWidth>=minw && theStage.stageWidth<=maxw)
                                {
                                        FitObjectArray[i].t.x = nextx + offsetx;
                                        isFit = true;
                                }
                                if (theStage.stageHeight >= minh && theStage.stageHeight <= maxh)
                                {
                                        FitObjectArray[i].t.y = nexty + offsety;
                                        isFit = true;
                                }
                                if (FitObjectArray[i].r && isFit)
                                {
